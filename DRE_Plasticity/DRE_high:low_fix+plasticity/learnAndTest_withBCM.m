...
% Collect Weights
inWCollect = zeros(ensembleDim,inputLength,initialRunlength + sampleRunlength + freeRunlength + plotRunlength);
outWCollect = zeros(outputLength,ensembleDim+inputLength,initialRunlength + sampleRunlength + freeRunlength + plotRunlength);
ofbWCollect = zeros(ensembleDim,outputLength,initialRunlength + sampleRunlength + freeRunlength + plotRunlength);
intWCollect = zeros(ensembleDim,ensembleDim,initialRunlength + sampleRunlength + freeRunlength + plotRunlength);

for i = 1:initialRunlength + sampleRunlength + freeRunlength + plotRunlength 
    
%     inWCollect[:,:,i] = inWM;   % collect input weights
%     outWCollect[:,:,i] = outWM; % collect output weights
%     ofbWCollect[:,:,i] = ofbWM; % collect output feedback weights
%     intWCollect[:,:,i] = intWM; % collect internal weights
...
Wcells = mat2cell(W,netDim*ones(1,reservoirNum),netDim*ones(1,reservoirNum));
SR = zeros(1,reservoirNum+1);
for iSR = 1:reservoirNum
    opts.disp = 0;
    SR(1,iSR) = abs(eigs(Wcells{iSR,iSR}, 1, 'lm', opts)); 
end
SR(1,reservoirNum+1) = abs(eigs(W, 1, 'lm', opts));
