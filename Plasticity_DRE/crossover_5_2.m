...
for i=1:nKids
    ...
    for j = 1:5
        if(rand > 0.5)
            xoverKids(i,((j-1)*7+1):((j-1)*7+5)) = thisPopulation(r1,((j-1)*7+1):((j-1)*7+5));
        else
            xoverKids(i,((j-1)*7+1):((j-1)*7+5)) = thisPopulation(r2,((j-1)*7+1):((j-1)*7+5));
        end
    end
    for j = 1:5
        if(rand > 0.5)
            xoverKids(i,(j*7-1):(7*j)) = thisPopulation(r1,(j*7-1):(7*j));
        else
            xoverKids(i,(j*7-1):(7*j)) = thisPopulation(r2,(j*7-1):(7*j));
        end
    end
