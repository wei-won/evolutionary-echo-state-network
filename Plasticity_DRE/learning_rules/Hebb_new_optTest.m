...
%% setup Hebb parameters
initialRun = 100;
% T = 100; % number of pattern presentations
...
%% reshape result mats
nrmseReshape = zeros(2*Llr,LT);
for i = 1:Llr
    nrmseReshape((i-1)*2+1,:) = nrmse((i-1)*LT+1:i*LT,1);
    nrmseReshape((i-1)*2+2,:) = nrmse((i-1)*LT+1:i*LT,2);
end

avg_nrmseReshape = reshape(avg_nrmse,LT,Llr)';

%% save results

% save('Hebb_optTest_results_gene40(2)WMs3.mat','avg_nrmse','nrmse','avg_nrmseReshape','nrmseReshape','gene','initialRunlength','seqlr','seqT');
